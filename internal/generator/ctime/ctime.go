...
// Package ctime provides strptime/strftime compatible syntax support
// for parsing and formatting time values with ctime-like format strings.
// e.g. "%Y-%m-%d %H:%M:%S %Z".
package ctime
...
// ctimeSubstitutes maps ctime format directives to Go time format directives
...
func ToNative(format string) (string, error) {
	...
	replaceFunc := func(directive string) string {
		...
		errs = append(errs, errors.New("unsupported ctime.ToNative() directive: "+directive))
		return ""
	}
