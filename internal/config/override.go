...
func otlpGrpcTLSOverrides() []*Override {
	return []*Override{
		{
			Field:   "output.otlp-grpc.enableTLS",
			Flag:    "output-otlp-grpc-enable-tls",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_ENABLE_TLS",
			Usage:   "enable TLS for OTLP gRPC connections",
			Default: false,
		},
		{
			Field:   "output.otlp-grpc.tls.insecure",
			Flag:    "otlp-grpc-tls-insecure",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_INSECURE",
		...
		{
			Field:   "output.otlp-grpc.insecure",
			Flag:    "otlp-grpc-tls-insecure",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_INSECURE",
			Usage:   "whether to use insecure credentials (no TLS) for OTLP gRPC connections",
			Default: true,
		...
		{
			Field:   "output.otlp-grpc.tls.cert",
			Flag:    "otlp-grpc-tls-cert",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_CERT",
			Usage:   "the path to the TLS certificate for OTLP gRPC connections",
			Default: "",
		},
		{
			Field:   "output.otlp-grpc.cert",
			Flag:    "otlp-grpc-tls-cert",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_CERT",
			Usage:   "the path to the TLS certificate for OTLP gRPC connections",
			Default: "",
		...
		{
			Field:   "output.otlp-grpc.tls.key",
			Flag:    "otlp-grpc-tls-key",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_KEY",
			Usage:   "the path to the TLS private key for OTLP gRPC connections",
			Default: "",
		},
		{
			Field:   "output.otlp-grpc.key",
			Flag:    "otlp-grpc-tls-key",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_KEY",
			Usage:   "the path to the TLS private key for OTLP gRPC connections",
			Default: "",
		...
		{
			Field:   "output.otlp-grpc.tls.ca",
			Flag:    "otlp-grpc-tls-ca",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_CA",
			Usage:   "the path to the TLS CA files. Optional, if not provided the host's root CA set will be used",
			Default: []string{},
		},
		{
			Field:   "output.otlp-grpc.ca",
			Flag:    "otlp-grpc-tls-ca",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_CA",
			Usage:   "the path to the TLS CA files. Optional, if not provided the host's root CA set will be used",
			Default: []string{},
		...
		{
			Field:   "output.otlp-grpc.tls.skipVerify",
			Flag:    "otlp-grpc-tls-skip-verify",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_SKIP_VERIFY",
			Usage:   "whether to skip TLS verification for OTLP gRPC connections",
			Default: false,
		},
		{
			Field:   "output.otlp-grpc.skipVerify",
			Flag:    "otlp-grpc-tls-skip-verify",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_SKIP_VERIFY",
			Usage:   "whether to skip TLS verification for OTLP gRPC connections",
			Default: false,
		...
		{
			Field:   "output.otlp-grpc.tls.minVersion",
			Flag:    "otlp-grpc-tls-min-version",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_MIN_VERSION",
			Usage:   "the minimum TLS version to use for OTLP gRPC connections. One of: 1.2|1.3",
			Default: "1.2",
		},
		{
			Field:   "output.otlp-grpc.minVersion",
			Flag:    "otlp-grpc-tls-min-version",
			Env:     "BLITZ_OUTPUT_OTLP_GRPC_TLS_MIN_VERSION",
			Usage:   "the minimum TLS version to use for OTLP gRPC connections. One of: 1.2|1.3",
			Default: "1.2",
...
func DefaultOverrides() []*Override {
	overrides := []*Override{
		...
		NewOverride("generator.palo-alto.workers", "number of palo-alto generator workers", 1),
		NewOverride("generator.palo-alto.rate", "rate at which palo-alto logs are generated per worker", 1*time.Second),
		...
		NewOverride("generator.hostmetrics.scrapers", "scrapers to enable (empty = all). One or more of: cpu|memory|disk|network|filesystem|load|paging|processes", []string{}),
		NewOverride("generator.traces.workers", "number of traces generator workers", 1),
		NewOverride("generator.traces.rate", "rate at which traces are generated per worker", 1*time.Second),
		NewOverride("generator.flow.workers", "number of flow generator workers", 1),
		NewOverride("generator.flow.rate", "rate at which flow records are generated per worker", 1*time.Second),
		NewOverride("generator.flow.scenario", "flow traffic-shape preset. One of: default|wan-edge|datacenter|ddos-target", DefaultFlowScenario),
		NewOverride("generator.flow.seed", "flow generator RNG seed (negative randomizes)", DefaultFlowSeed),
		NewOverride("generator.wel.workers", "number of WEL generator workers", 1),
		NewOverride("generator.wel.rate", "rate at which WEL events are generated per worker", 1*time.Second),
		NewOverride("generator.wel.channels", "WEL channels to generate events for (empty = all)", []string{}),
		NewOverride("generator.wel.computer", "computer name for WEL events (empty = auto)", ""),
		NewOverride("generator.wel.domain", "domain name for WEL events", "WORKGROUP"),
		NewOverride("generator.wel.role", "WEL machine role. One of: workstation|member|dc", "member"),
		NewOverride("generator.wel.manageEventSources", "auto-register/deregister WEL event sources (requires admin)", true),
		...
		NewOverride("output.otlp-grpc.host", "OTLP gRPC output target host", DefaultOTLPGrpcHost),
		NewOverride("output.otlp-grpc.port", "OTLP gRPC output target port", DefaultOTLPGrpcPort),
		NewOverride("output.otlp-grpc.workers", "number of OTLP gRPC output workers", DefaultOTLPGrpcWorkers),
		NewOverride("output.otlp-grpc.batchTimeout", "OTLP gRPC output batch timeout", DefaultOTLPGrpcBatchTimeout),
		NewOverride("output.otlp-grpc.requestTimeout", "timeout for each OTLP gRPC export call", DefaultOTLPGrpcRequestTimeout),
		NewOverride("output.otlp-grpc.maxQueueSize", "OTLP gRPC output maximum queue size", DefaultOTLPGrpcMaxQueueSize),
		NewOverride("output.otlp-grpc.maxExportBatchSize", "OTLP gRPC output maximum export batch size", DefaultOTLPGrpcMaxExportBatchSize),
		...
		NewOverride("output.flow.host", "flow collector host", DefaultFlowOutputHost),
		NewOverride("output.flow.port", "flow collector UDP port", DefaultFlowOutputPort),
		NewOverride("output.flow.protocol", "flow wire format. One of: netflow-v5|netflow-v9|ipfix|sflow", DefaultFlowProtocol),
		NewOverride("output.flow.vendor", "optional flow vendor flavor. One of: jflow|netstream|cflow|appflow|rflow", ""),
		NewOverride("output.flow.agentIP", "sFlow exporter agent IP (sflow only)", ""),
