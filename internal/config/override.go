...
func DefaultOverrides() []*Override {
	overrides := []*Override{
		...
		NewOverride("generator.type", "generator type. One of: nop|json|winevt|palo-alto|apache-common|apache-combined|apache-error|nginx|postgres|kubernetes|filegen|okta|metrics", GeneratorTypeNop),
		...
		NewOverride("generator.metrics.workers", "number of metrics generator workers", 1),
		NewOverride("generator.metrics.rate", "rate at which metrics are generated per worker", 60*time.Second),
