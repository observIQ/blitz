...
func TestForEmbedRejectsNilLogger(t *testing.T) {
	_, err := ForEmbed(nil, config.Generator{Type: config.GeneratorTypeWel}, noopConsumer{}, nil)
	require.Error(t, err)
}

func TestForEmbedRejectsNilConsumer(t *testing.T) {
	_, err := ForEmbed(zap.NewNop(), config.Generator{Type: config.GeneratorTypeWel}, nil, nil)
	require.Error(t, err)
}
