...
func TestApacheCombinedGenerator_Name(t *testing.T) {
	logger := zaptest.NewLogger(t)
	generator, err := New(logger, 1, 100*time.Millisecond, newMockConsumer())
...
var _ embed.ProducerModule = (*ApacheCombinedLogGenerator)(nil)

func TestApacheCombinedGenerator_Start(t *testing.T) {
	logger := zaptest.NewLogger(t)
	...
	// Verify Combined Log Format - should have referer and user-agent
	combinedPattern := regexp.MustCompile(`^\d+\.\d+\.\d+\.\d+ - - \[.*\] ".*" \d+ \d+ ".*" ".*"$`)
	for _, write := range writes {
		line := string(write)
		assert.True(t, combinedPattern.MatchString(line), "Log should match Combined Log Format: %s", line)
		// Verify it contains referer and user-agent (quoted strings at the end)
		assert.Contains(t, line, `"`, "Log should contain quoted referer and user-agent")
	}
}

func TestApacheCombinedGenerator_Stop_GracefulShutdown(t *testing.T) {
	logger := zaptest.NewLogger(t)
	consumer := newMockConsumer()
...
func TestApacheCombinedGenerator_WriteErrors_Backoff(t *testing.T) {
	logger := zaptest.NewLogger(t)
	consumer := newMockConsumer()
...
func TestApacheCombinedGenerator_ConcurrentWorkers(t *testing.T) {
	logger := zaptest.NewLogger(t)
	consumer := newMockConsumer()
...
func TestFormatAsApacheCombined_Structure(t *testing.T) {
	logger := zaptest.NewLogger(t)
	consumer := newMockConsumer()
	generator, err := New(logger, 1, 10*time.Millisecond, consumer)
	require.NoError(t, err)
	...
	// Verify Combined Log Format structure
	line := string(writes[0])
	parts := strings.Fields(line)
	assert.GreaterOrEqual(t, len(parts), 9, "Combined Log Format should have at least 9 fields")
	...
	// Verify referer and user-agent are quoted
...
func TestFormatAsApacheCombined_ParseFunc(t *testing.T) {
	logger := zaptest.NewLogger(t)
	consumer := newMockConsumer()
	generator, err := New(logger, 1, 10*time.Millisecond, consumer)
	require.NoError(t, err)
	...
	line := string(writes[0])
	parts := strings.Fields(line)
	assert.GreaterOrEqual(t, len(parts), 9, "Combined Log Format should have enough fields to parse")
}
...
func TestApacheCombinedLogGenerator_SetCountTracker(t *testing.T) {
	logger := zaptest.NewLogger(t)
...
func TestApacheCombinedLogGenerator_CountLimited(t *testing.T) {
	logger := zaptest.NewLogger(t)
	consumer := newMockConsumer()
