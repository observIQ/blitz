...
func init() {
	systemEvents := []struct {
		...
		{4697, taskSecuritySystemExt, keywordsAuditSuccess, generateSecSystemServiceInstalled},
...
func generateSecSystemServiceInstalled(r *rand.Rand, opts *GenerateOpts) ([]EventDataField, string) {
	svcName := "TestService"
	data := append(subjectFields(r, opts),
		EventDataField{Name: "ServiceName", Value: svcName},
		EventDataField{Name: "ServiceFileName", Value: `C:\Program Files\Service\svc.exe`},
		EventDataField{Name: "ServiceType", Value: "0x10"},
		EventDataField{Name: "ServiceStartType", Value: "0x2"},
		EventDataField{Name: "ServiceAccount", Value: "LocalSystem"},
	)
	msg := fmt.Sprintf("A service was installed in the system.\n\nSubject:\n\tAccount Name:\t\t%s\n\nService:\n\tService Name:\t%s",
		data[1].Value, svcName)
	return data, msg
}
